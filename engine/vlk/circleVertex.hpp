...
struct CircleVertex {
  glm::vec3 worldPos; // this might break shit
  float radius;
  glm::vec3 color;
  ...
  getAttributeDescriptions() {
    ...
    attributeDesc[1].format = VK_FORMAT_R32_SFLOAT;
    attributeDesc[1].offset = offsetof(CircleVertex, radius);
    ...
    attributeDesc[3].format = VK_FORMAT_R32_UINT;
    attributeDesc[3].offset = offsetof(CircleVertex, entityID);

    attributeDesc[4].binding = 0;
    attributeDesc[4].location = 4;
    attributeDesc[4].format = VK_FORMAT_R32G32_SFLOAT; // vec2
    attributeDesc[4].offset = offsetof(CircleVertex, localPos);
